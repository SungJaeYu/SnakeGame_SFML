...
void World::Update(Snake& l_player)
{
    ...
    auto playerPosition = l_player.GetPosition();

    if (playerPosition.x <= 0 || playerPosition.y <= 0 || playerPosition.x >= gridSize_x - 1 || playerPosition.y >= gridSize_y - 1) {
        l_player.Lose();
    }
}
